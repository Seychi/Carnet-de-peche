...
> ✅ **Lot 6 (Corse 2A/2B) INSÉRÉ EN PROD le 2026-06-22 → 157 spots = COUVERTURE NATIONALE COMPLÈTE** (5 façades : Bretagne, Atlantique, Manche, Méditerranée continentale, Corse). 18 spots (2B=9, 2A=9), SQL `supabase/seed-spots-lot-6.sql` (joué via MCP execute_sql, OK John ; `region='corse'`, espèces sans maquereau). Vérif : `docs/sprint-10/lot-6-verification.md`. **Garde-fou 2A/2B vérifié VERT** (departments/centroids/`/fil`/`can_post_in_department`/char(3)/filtre dynamique — aucune migration). Satellite 2 passes : 12 OK + **6 hors-structure CORRIGÉES** (« erreur corse » : phare/tour génoise/vieille ville perchée au lieu du poste). Flou GPS 522-887 m, `anon` ne lit pas `geom`. **Reste : revue `/carte` + `verified=true` par John. Plus de lot de curation prévu (objectif 100+ largement tenu).**

> ✅ **Lot 5 (Méditerranée) INSÉRÉ EN PROD le 2026-06-22 → 139 spots** : 30 spots (66=4, 11=3, 34=5, 30=2, 13=6, 83=6, 06=4), **3ᵉ façade** (Méditerranée continentale ; Corse = lot 6 à venir). SQL : `supabase/seed-spots-lot-5.sql` (joué via MCP execute_sql, OK John ; `verified=false`, `region` `occitanie`/`provence-alpes-cote-d-azur`). Vérif : `docs/sprint-10/lot-5-verification.md`. Espèces Med-correctes (zéro lieu_jaune/vieille), zéro `submersion_maree` (pas de marnage), Parc National des Calanques (#15/#16) + Parc Marin Côte Bleue (#17/#18) + rappel arrêté portuaire signalés. Satellite **2 passes** : 18 coords OK + 1 review gardée + **11 coords hors-structure CORRIGÉES + 1 (#28 embouchure du Var) déplacée** — cause = nœuds OSM pointant l'objet NOMMÉ (phare au sommet, village perché, centre-ville, mont) au lieu du poste au bord de l'eau. Décisions John : 12 coords validées + #20 plage Napoléon → `plage`. Flou GPS vérifié **503-866 m**. **Reste : revue `/carte` + `verified=true` par John.**
>
