...
export default function HomePage() {
  return (
    <div className="bg-sand-50 text-ink-900 font-sans">
      ...
      >
        <div className="mx-auto max-w-[1200px] px-5 flex items-center justify-between gap-4 h-[68px]">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-2.5 font-display font-bold text-[17px] text-navy-900 shrink-0 min-h-[44px] min-w-[44px]">
            <BrandMark size={34} />
            <span className="hidden xs:inline sm:inline">Carnet de Pêche</span>
          </Link>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <Link
              href="/auth/login"
              className="hidden sm:inline-flex items-center px-4 py-2.5 rounded-full text-[14px] font-semibold text-navy-900 border-[1.5px] border-ink-200 min-h-[44px]"
            >
              Connexion
            </Link>
            <Link
              href="/auth/register"
              className="inline-flex items-center px-4 py-2.5 rounded-full text-[14px] font-semibold min-h-[44px] whitespace-nowrap"
              style={{ background: "var(--navy-900)", color: "#fff", boxShadow: "0 4px 14px rgba(10,47,61,.18)" }}
            >
              Créer mon carnet
            </Link>
            <MobileNav />
        ...
        <div className="mx-auto max-w-[1200px] px-5">
          ...
          >
            ...
            <p className="relative mt-3 mb-7 max-w-[560px] mx-auto text-[15px] sm:text-[16px] text-white/80 leading-relaxed">
              Démarre avec ton département gratuit pendant 14 jours. Carnet illimité, carte intelligente, communauté locale. Sans carte bancaire pour la version Découverte.
            </p>
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────────── */}
      <footer className="bg-navy-950 text-white/70 pt-12 pb-8 lg:pt-20">
        <div className="mx-auto max-w-[1200px] px-5">
          {/* Grille : 2 cols mobile, 4 cols desktop */}
          <div className="grid grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_1fr] gap-8 lg:gap-12 pb-10 border-b border-white/10">
            {/* Brand — pleine largeur sur mobile */}
            <div className="col-span-2 lg:col-span-1">
              <div className="flex items-center gap-2.5 font-display text-white text-[18px] font-bold mb-3">
                <BrandMark size={32} />
                Carnet de Pêche
              </div>
              <p className="text-[14px] text-white/60 max-w-[300px] leading-relaxed">
                Le réseau des pêcheurs à la canne du bord. Fait par un pêcheur, pour les pêcheurs.
              </p>
            </div>

            {/* Colonnes de liens */}
            {[
              { title: "Produit", links: ["Carte intelligente", "Carnet de pêche", "Communauté", "App iOS / Android"] },
              { title: "Ressources", links: ["Guides espèces", "Régions", "Réglementation", "Blog"] },
              { title: "Société", links: ["L'histoire", "Charte communauté", "Contact", "Mentions légales"] },
            ].map((col) => (
              <div key={col.title}>
                <h3 className="text-white text-[12px] font-semibold tracking-[.06em] uppercase mb-4 leading-none">
                  {col.title}
                </h3>
                <div className="flex flex-col gap-0">
                  {col.links.map((link) => (
                    <Link
                      key={link}
                      href="#"
                      className="text-[13px] sm:text-[14px] text-white/65 hover:text-teal-400 transition-colors py-2.5 min-h-[44px] flex items-center"
                    >
                      {link}
                    </Link>
                  ))}
          ...
          {/* Meta */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-8 text-[12px] text-white/40">
            <div>© 2026 Carnet de Pêche. « Logue. Partage. Progresse. »</div>
            <div>CGU · Confidentialité · RGPD</div>
          </div>
        </div>
      </footer>
