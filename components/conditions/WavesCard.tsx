...
export default function WavesCard({ waves, swell }: Props) {
  ...
  return (
    <div className="bg-white border border-ink-100 rounded-[14px] p-5">
      <div className="flex items-center gap-2 mb-4">
        <Waves size={16} className="text-teal-500" />
        <span className="text-xs font-semibold uppercase tracking-wide text-ink-500">Vagues</span>
      </div>
          ...
          <dl className="flex flex-col gap-2 text-sm">
            ...
            {waves.period_s !== null && (
              <div className="flex justify-between">
                ...
                <dd className="font-medium text-ink-900">{Math.round(waves.period_s)} s</dd>
              </div>
            )}
            {waves.water_temp_c !== null && (
              <div className="flex justify-between">
                <dt className="text-ink-500">Temp. eau</dt>
                <dd className="font-medium text-ink-900">{Math.round(waves.water_temp_c)}°C</dd>
              </div>
            )}
          </dl>
            ...
            <div className="mt-4 pt-4 border-t border-ink-100">
              ...
              <dl className="flex flex-col gap-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium text-ink-900">{swell.height_m!.toFixed(1)} m</dd>
                </div>
                {swell.period_s !== null && (
                  <div className="flex justify-between">
                    <dt className="text-ink-500">Période</dt>
                    <dd className="font-medium text-ink-900">{Math.round(swell.period_s)} s</dd>
                  </div>
                )}
              </dl>
            </div>
